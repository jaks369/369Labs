...
/**
 * Returns true only for Deriv synthetic/jump indices whose prices are
 * generated by a random-number engine and whose last digit is engineered
 * to be uniform across 0–9. Digit-pattern analysis (Matches/Differs/
 * Over/Under/Even/Odd) is ONLY statistically valid on these symbols.
 *
 * Real forex/commodities/crypto/stock-index prices have no such property —
 * running digit-pattern tests against them produces spurious, extreme-looking
 * "edges" that are statistical artifacts, not real signals.
 */
export function isSyntheticIndexSymbol(symbol: string): boolean {
