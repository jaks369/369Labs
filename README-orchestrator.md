# Free-tier orchestrator (Planner → Builder → Reviewer, optional Researcher)
...
```bash
# Gemini CLI — free tier, personal Google account, no card required
npm install -g @google/gemini-cli
gemini auth login   # or however the current installer prompts you

# OpenCode CLI — free, open-source
curl -fsSL https://opencode.ai/install | bash
opencode auth login
opencode models      # see what's actually available/free on your account
```

Command names and flags for both tools move fast — if something in
...
Flags:
- `--research` — runs a Researcher pass with Gemini before planning
- `--skip-review` — skips the Reviewer pass
- `--builder-model provider/model` — override the OpenCode model (default: `opencode/grok-code`, a free tier)
- `--dry-run` — prints what would be called without actually running anything

## 3. What it does

1. **(optional) Researcher** — Gemini CLI gathers background/context
2. **Planner** — Gemini CLI turns your goal into a numbered implementation plan
3. **Builder** — OpenCode CLI executes the plan, editing files in your repo directly
4. **Reviewer** — Gemini CLI checks the build log against the plan and flags gaps

Every step's output is saved to `./orchestrator-runs/<timestamp>/` so you
have a record — this is a manual stand-in for Ruflo's memory layer, not the
...
a retry loop if Reviewer flags problems, or loop Builder→Reviewer a few times).
...
OpenCode at it instead of a cloud model — check `opencode models` after
