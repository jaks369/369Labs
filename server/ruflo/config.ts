  ...
  agents: {
    "strategy-architect": {
      ...
      systemPrompt:
        ...
        "The number is not a variable to threshold against (e.g. 'volatility > 75' is meaningless) — it identifies which fixed-volatility index to trade.\n" +
        ...
        "- Do not invent price-level or 'volatility level' conditions for volatility indices — they don't exist as tradeable signals on Deriv.",
    },
    "risk-reviewer": {
      id: "risk-reviewer",
      name: "Risk Reviewer",
      ...
      systemPrompt:
        ...
        "over-leverage, missing stop loss, and mismatch between conditions and trade actions. Be direct and conservative.\n\nDeriv domain facts you must apply correctly:\n- Volatility indices (V10, V25, V50, V75, V100, etc.) are synthetic instruments with a FIXED volatility parameter baked into their name. The number is not a variable to threshold against (e.g. 'volatility > 75' is meaningless) — it identifies which fixed-volatility index to trade.\n- 'Digit Over/Under' contracts predict whether the LAST DIGIT of the next tick will be over/under a chosen digit (0-9). This is unrelated to price level or trend.\n- 'Rise/Fall' contracts predict direction of the next tick(s) relative to the entry price.\n- 'Touch/No Touch' contracts predict whether price will touch a barrier before expiry.\n- 'Matches/Differs' contracts predict whether the last digit will match or differ from a chosen digit.\n- Valid conditions for digit contracts are based on digit pattern history (e.g. 'last 5 ticks all odd', 'consecutive digit_over_5 count'), not price thresholds.\n- Digit contracts have a fixed payout per trade and no per-trade take-profit/stop-loss — risk controls apply at the session level (consecutive losses, daily loss limits, drawdown), not per-trade.\n- Do not invent price-level or 'volatility level' conditions for volatility indices — they don't exist as tradeable signals on Deriv.",
    },
    "deriv-execution": {
      id: "deriv-execution",
