...
async function ensureDerivWS() {
  ...
  await new Promise((res, rej) => { derivConnection.onopen = res; derivConnection.onerror = rej; });
  return derivConnection;
}

async function sendDeriv(msg: any): Promise<any> {
...
export async function getTickHistory(symbol: string, count: number = 100) {
  const now = Math.floor(Date.now());
  const start = now - count * 2 * 1000;
  const res = await sendDeriv({ ticks_history: symbol, end: "latest", start: Math.floor(start / 1000), adjust_start_time: 1, count });
  if (res.error) throw new Error(res.error.message);
  return (res.history?.prices || []).map((p: number, i: number) => ({ price: p, timestamp: res.history?.times?.[i] * 1000 || 0 }));
}
