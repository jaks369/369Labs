  ...
  async getRiskBehaviour(userId: number): Promise<RiskBehaviourData> {
    try {
      ...
      const streakSummary = total > 0
        ? `${wins}W / ${losses}L (${Math.round((wins / total) * 100)}%) — PnL: ${pnl >= 0 ? "+" : ""}${Math.round(pnl * 100) / 100}`
        : "No completed trades";
  ...
  async getRecommendations(userId: number): Promise<string[]> {
    try {
      ...
      if (worstSymbol && worstSymbol[1].accuracyPct < 50) {
        recs.push(`Consider avoiding ${worstSymbol[0]} — accuracy is only ${worstSymbol[1].accuracyPct}% over ${worstSymbol[1].total} predictions.`);
      }
      ...
      if (recentLosses.length >= 3) {
        recs.push(`Recent performance suggests reducing exposure — ${recentLosses.length} recent losses detected.`);
      }
      ...
      if (worstPnl && worstPnl[1].pnl < -10) {
        recs.push(`Net loss of ${Math.abs(worstPnl[1].pnl).toFixed(1)} on ${worstPnl[0]} — consider reduced allocation.`);
      }
