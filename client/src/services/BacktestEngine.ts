...
function lastDigit(price: number): number {
  return parseInt(price.toFixed(2)[price.toFixed(2).length - 1], 10);
}

function evaluateCondition(rule: StrategyRule, tick: Tick, history: Tick[]): boolean {
  const indicator = rule.condition.indicator;
  const count = rule.condition.count;
  if (history.length < count) return false;

  // Check if condition was met at a specific index in history
...
export async function runBacktest(
  ...
  switch (strategy.action.tradeType) {
    case "buy_rise": contractType = "CALL"; break;
    case "buy_fall": contractType = "PUT"; break;
    case "buy_even": contractType = "DIGITEVEN"; break;
    case "buy_odd": contractType = "DIGITODD"; break;
    case "buy_over": contractType = "DIGITOVER"; barrier = strategy.condition.barrier ?? 5; break;
    case "buy_under": contractType = "DIGITUNDER"; barrier = strategy.condition.barrier ?? 5; break;
    default: contractType = "CALL";
  }
