    ...
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 w-80 max-w-[90vw]">
      {items.map((t) => (
        <div
          ...
          className={`cursor-pointer bg-[#151B23] border ${t.kind === "success" ? "border-green-500/50" : t.kind === "error" ? "border-red-500/50" : t.kind === "info" ? "border-blue-500/50" : t.kind === "warning" ? "border-yellow-500/50" : "border-[#252B35]"} rounded-lg px-4 py-3 text-sm font-medium shadow-lg shadow-black/40 animate-[fadeIn_0.2s_ease] flex items-center gap-2`}
        >
