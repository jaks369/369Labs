    ...
    <div className="min-h-screen bg-[#0B0F14] text-[#F59E0B] p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-[#FBBF24] mb-2">TRADE HISTORY</h1>
            <p className="text-[#F59E0B] text-sm">View all your executed trades</p>
          </div>
          <button
            onClick={exportToCSV}
        ...
        <div className="flex gap-2 mb-6">
          <button onClick={() => setTab("trades")} className={`px-4 py-2 rounded-lg text-sm font-bold ${tab === "trades" ? "bg-[#FBBF24]/20 text-[#FBBF24]" : "text-[#F59E0B]/60 hover:text-[#F59E0B]"}`}>TRADES</button>
          <button onClick={() => setTab("prices")} className={`px-4 py-2 rounded-lg text-sm font-bold ${tab === "prices" ? "bg-[#FBBF24]/20 text-[#FBBF24]" : "text-[#F59E0B]/60 hover:text-[#F59E0B]"}`}>PRICE HISTORY</button>
        </div>
        ...
        <div className="hud-panel p-6 mb-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h2 className="text-lg font-bold text-[#22D3EE]">AI TRADING JOURNAL</h2>
              <p className="text-xs text-[#22D3EE]/60">Automatic post-trade analysis — why you won, why you lost, and how to improve.</p>
            </div>
            <button
          ...
          {journalMutation.data ? (
            <div className="space-y-3">
              ...
              <p className="text-sm text-[#22D3EE]/90 whitespace-pre-wrap leading-relaxed">{journalMutation.data.analysis}</p>
            </div>
          ) : journalMutation.isPending ? (
            <div className="flex items-center gap-2 text-sm text-[#22D3EE]/60"><Loader2 className="w-4 h-4 animate-spin" /> Analyzing your trades...</div>
          ) : (
            <p className="text-sm text-[#22D3EE]/50">Press ANALYZE to generate an AI journal from your recent trades.</p>
          )}
        </div>
        ...
        {tab === "prices" ? (
          <div className="hud-panel p-6">
            <div className="flex items-center gap-3 mb-4">
              <label className="text-xs text-[#F59E0B]/60">SYMBOL</label>
              <select value={priceSymbol} onChange={(e) => setPriceSymbol(e.target.value)} className="bg-[#151B23] border border-[#F59E0B]/30 rounded px-3 py-2 text-sm text-[#F59E0B]">
            ...
            {priceQuery.isLoading ? (
              <div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-[#F59E0B]" /></div>
            ) : priceQuery.data?.ticks?.length ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#F59E0B]/30">
                      <th className="text-left py-3 px-4 text-[#FBBF24] font-bold">TIME</th>
                      <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">PRICE</th>
                      <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">LAST DIGIT</th>
                    </tr>
                  </thead>
                  <tbody>
                    {priceQuery.data.ticks.slice().reverse().map((t: any, i: number) => (
                      <tr key={i} className="border-b border-[#F59E0B]/10">
              ...
              <div className="text-center py-12 text-[#F59E0B]/60">No price history found.</div>
            )}
          </div>
        ) : (
        <div className="hud-panel overflow-x-auto">
          {tradesQuery.isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#F59E0B]/30">
                  <th className="text-left py-3 px-4 text-[#FBBF24] font-bold">ID</th>
                  <th className="text-left py-3 px-4 text-[#FBBF24] font-bold">ENTRY TIME</th>
                  <th className="text-left py-3 px-4 text-[#FBBF24] font-bold">EXIT TIME</th>
                  <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">ENTRY PRICE</th>
                  <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">EXIT PRICE</th>
                  <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">STAKE</th>
                  <th className="text-right py-3 px-4 text-[#FBBF24] font-bold">P&L</th>
                  <th className="text-center py-3 px-4 text-[#FBBF24] font-bold">RESULT</th>
                </tr>
              </thead>
              <tbody>
                {tradesQuery.data.map((trade) => (
                  <tr key={trade.id} className="border-b border-[#F59E0B]/10 hover:bg-[#151B23]/50 transition">
            ...
            <div className="text-center py-12 text-[#F59E0B]/60">
        ...
        {tradesQuery.data && tradesQuery.data.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8">
            <div className="hud-card-pink">
              <div className="text-sm text-[#FBBF24] mb-2">TOTAL TRADES</div>
              <div className="text-2xl font-bold text-[#FBBF24]">{tradesQuery.data.length}</div>
            </div>
            <div className="hud-card">
              <div className="text-sm text-[#F59E0B] mb-2">WIN RATE</div>
              <div className="text-2xl font-bold text-[#22C55E]">
                {((tradesQuery.data.filter(t => t.result === "win").length / tradesQuery.data.length) * 100).toFixed(1)}%
              </div>
            </div>
            <div className="hud-card">
              <div className="text-sm text-[#F59E0B] mb-2">TOTAL P&L</div>
            ...
            <div className="hud-card">
              <div className="text-sm text-[#F59E0B] mb-2">AVG TRADE</div>
              <div className={`text-2xl font-bold ${
