...
const ACCENT = "text-[#D98B1F]";
const ACCENT_BG = "bg-[#D98B1F]/10";
const ACCENT_BORDER = "border-[#D98B1F]/30";
...
export default function AIAssistant() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] lg:h-screen bg-[#151515]">
      <div className="p-6 border-b border-[#2A2A2A] flex items-center justify-between bg-[#151515]/50 backdrop-blur-xl">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`flex gap-4 max-w-[80%] ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${msg.role === "ai" ? `${ACCENT_BG} ${ACCENT_BORDER}` : "bg-[#151515] border-[#2A2A2A]"}`}>
                ...
                <div className={`p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-line ${msg.role === "ai" ? "bg-[#151515] border border-[#2A2A2A] text-[#A8A8A8] border-l-2 border-l-amber-400/60" : "bg-[#D98B1F] text-black font-medium"}`}>
                  {msg.content}
                  {msg.steps && msg.steps.length > 0 && (
                    <div className="mt-3 border-t border-[#2A2A2A] pt-2">
                      <button onClick={() => setExpanded(e => ({ ...e, [i]: !e[i] }))} className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-[#6F6F6F] hover:text-[#D98B1F]">
                      ...
                      {expanded[i] && (
                        <div className="mt-2 space-y-1 text-[11px] font-mono">
                          {msg.steps.map((s: any, j: number) => (
                            <div key={j} className="rounded bg-black/40 p-2 border border-[#2A2A2A]">
                              <span className="text-[#22C55E]">{">"} {s.tool}</span>
                              <span className="text-[#6F6F6F]">({JSON.stringify(s.args)})</span>
                              <span className="text-[#D98B1F]"> {"=>"} {s.result?.__action ? "ACTION" : "ok"}</span>
                            </div>
                          ))}
                        </div>
                      )}
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="flex gap-4 max-w-[80%]">
                ...
                <div className="p-4 rounded-2xl bg-[#151515] border border-[#2A2A2A] flex items-center gap-2 text-xs text-[#D98B1F] font-mono">
      ...
      {pending && (
        <div className="p-4 border-t border-[#D98B1F]/30 bg-[#D98B1F]/5">
          <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
            <div className="text-xs text-[#E89A2A] font-mono">
              <span className="font-bold uppercase">{pending.action}</span> {JSON.stringify(pending.params)}
            </div>
            <div className="flex gap-2">
              <button onClick={() => setPending(null)} className="px-3 py-1.5 rounded bg-[#151515] text-[#A8A8A8] text-xs hover:bg-[#2A2A2A]">Cancel</button>
              <button onClick={executeAction} className="px-3 py-1.5 rounded bg-[#D98B1F] text-black text-xs font-bold hover:bg-[#D98B1F]">Confirm</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {messages.length < 3 && (
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              {suggestions.map(s => (
                <button key={s} onClick={() => setInput(s)} className="whitespace-nowrap px-4 py-2 rounded-full bg-[#151515] border border-[#151515] text-xs text-[#A8A8A8] hover:border-[#D98B1F] hover:text-[#E89A2A] transition-all">{s}</button>
              ))}
            </div>
          )}
          <div className="relative">
            <textarea
              ...
              className="w-full bg-[#151515] border-[#2A2A2A] rounded-xl pl-4 pr-12 py-4 text-sm focus:border-[#D98B1F] focus:ring-1 focus:ring-[#D98B1F] transition-all resize-none h-14"
            />
            <button onClick={handleSend} disabled={!input.trim() || isTyping} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-[#D98B1F] text-black rounded-lg hover:bg-[#D98B1F] disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              <Send className="w-4 h-4" />
            </button>
          </div>
          <div className="flex items-center justify-center gap-3 text-[10px] font-bold uppercase tracking-widest">
            <button onClick={() => handleSend("Give me a live market analysis: pick an active volatility symbol, read its recent ticks, and tell me the current trend, hottest/odd last digits, and any repeatable pattern forming right now.")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#151515] border border-[#151515] text-[#A8A8A8] hover:border-[#D98B1F] hover:text-[#E89A2A] transition-all">
              <LineChart className="w-3 h-3" /> Market Analysis
            </button>
            <button onClick={() => handleSend("What is my risk on the current bots and open positions? Recommend stake sizing, stop-loss and take-profit rules based on the volatility symbols I am trading.")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#151515] border border-[#151515] text-[#A8A8A8] hover:border-[#D98B1F] hover:text-[#E89A2A] transition-all">
