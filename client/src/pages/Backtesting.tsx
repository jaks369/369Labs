      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[#94A3B8] text-sm mt-1">Test your strategies against historical market data from Deriv</p>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-[#151B23] border border-[#252B35] rounded-xl p-6 space-y-4">
            <h2 className="text-lg font-bold text-white">Parameters</h2>

            <div>
              <label className="text-xs text-[#64748B] font-bold uppercase tracking-wider">Symbol</label>
              <select value={symbol} onChange={e => setSymbol(e.target.value)} className="w-full mt-1 bg-[#151B23] border border-[#252B35] rounded-lg px-3 py-2 text-white text-sm">
            ...
            <select value={selectedStrategyId || ""} onChange={e => setSelectedStrategyId(Number(e.target.value))} className="w-full mt-1 bg-[#151B23] border border-[#252B35] rounded-lg px-3 py-2 text-white text-sm">
              ...
              {(!strategiesQuery.data || strategiesQuery.data.length === 0) && (
                <p className="text-xs text-[#64748B] mt-1">No strategies found. Save one in Strategy Builder first.</p>
              )}
            </div>

            <div>
              <label className="text-xs text-[#64748B] font-bold uppercase tracking-wider">Start Date</label>
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full mt-1 bg-[#151B23] border border-[#252B35] rounded-lg px-3 py-2 text-white text-sm" />
            </div>

            <div>
              <label className="text-xs text-[#64748B] font-bold uppercase tracking-wider">End Date</label>
              <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full mt-1 bg-[#151B23] border border-[#252B35] rounded-lg px-3 py-2 text-white text-sm" />
            </div>

            <div>
              <label className="text-xs text-[#64748B] font-bold uppercase tracking-wider">Stake ($)</label>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#151B23] border border-[#252B35] rounded-xl p-4">
                    <p className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider">Total Trades</p>
                      ...
                      <tbody className="divide-y divide-[#252B35]">
                        {result.trades.slice(0, 100).map((t, i) => (
                          <tr key={i} className="hover:bg-white/5">
                            ...
                            <td className="py-3 text-[#94A3B8]">${t.entryPrice.toFixed(2)}</td>
                ...
                <div className="bg-[#151B23] border border-[#252B35] rounded-xl p-6">
                  ...
                  <p className="text-xs text-[#64748B] mb-4">Stress-test one parameter across a range on the same tick window. Cells show win rate (green = strong, red = weak).</p>
                  <div className="flex items-center gap-2 mb-4">
                    <select value={sweepParam} onChange={(e) => setSweepParam(e.target.value as any)} className="bg-[#151B23] border border-[#252B35] rounded-lg px-3 py-2 text-white text-sm">
            ...
            {!result && !running && !error && (
              <div className="bg-[#151B23] border border-[#252B35] rounded-xl p-8 flex items-center justify-center h-64">
                <div className="text-center">
                  <BarChart3 className="w-12 h-12 mx-auto mb-4 text-[#252B35]" />
                  <p className="text-[#64748B]">Configure parameters and run a backtest to see results</p>
                </div>
              </div>
            )}
          </div>
