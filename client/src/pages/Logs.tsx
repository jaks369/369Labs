          ...
          <div className="bg-[var(--card)] border border-[var(--red)]/30 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-[var(--red)]" /> Application Errors ({appErrors.length})
              </h2>
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto font-mono text-xs">
              {[...appErrors].reverse().slice(0, 100).map((e) => (
                <div key={e.id} className="flex items-start gap-2 p-2 bg-[var(--red-soft)] rounded-lg border border-[var(--red)]/10">
                  <AlertTriangle className="w-3.5 h-3.5 text-[var(--red)] mt-0.5 shrink-0" />
                  <span className="text-[var(--red)] flex-1">{e.message}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-6">
            <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
              <Activity className="w-4 h-4 text-[var(--accent)]" /> Live Agent Activity
            </h2>
            <AITimeline />
          </div>
          ...
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-6">
            ...
            {logsQuery.isLoading ? (
              <div className="flex items-center justify-center py-12"><Loader2 className="w-6 h-6 animate-spin text-[var(--accent)]" /></div>
            ) : logsQuery.isError ? (
