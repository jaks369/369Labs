    ...
    for (const step of w.steps) {
      ...
      try {
        if (step.kind === "scan" || step.kind === "watch") {
          ...
          try {
            const signals = await utils.client.signals.list.query({ symbol: sym, limit: 5 });
          ...
          try {
            const strat = await utils.client.strategies.save.mutateAsync({ name: `Workflow - ${w.name}`, description: `Auto-generated by workflow "${w.name}" on ${sym}`, config: { rule: { symbol: sym } }, isActive: false, published: false });
