    ...
    for (const step of w.steps) {
      ...
      try {
        if (step.kind === "scan" || step.kind === "watch") {
          ...
          try {
            const strat = await utils.client.strategies.save.mutateAsync({ name: `Workflow - ${w.name}`, description: `Auto-generated by workflow "${w.name}" on ${sym}`, config: { rule: { symbol: sym } }, published: false });
