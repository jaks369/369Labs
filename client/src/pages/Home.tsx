...
import { Brain, Globe, BarChart3, ChevronRight, Activity, Star, CheckCircle2, Zap, Shield, TrendingUp, ArrowRight } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-primary)] selection:bg-[var(--amber)]/30">
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-[var(--amber)] blur-[200px] rounded-full opacity-10" />
      </div>

      <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="relative z-50 border-b border-[var(--border)]/50 sticky top-0 bg-[var(--bg)]/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 h-14 flex justify-between items-center">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
              <div className="w-7 h-7 bg-[var(--amber)] rounded-lg flex items-center justify-center">
            ...
            <div className="hidden md:flex items-center gap-6">
              {[{ label: "Dashboard", path: "/dashboard" }, { label: "Strategy Builder", path: "/strategy-builder" }, { label: "Marketplace", path: "/marketplace" }, { label: "Backtesting", path: "/backtesting" }].map((item) => (
                <button key={item.path} onClick={() => navigate(item.path)} className="text-sm text-[var(--text-muted)] hover:text-white transition-colors">{item.label}</button>
              ))}
            </div>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => navigate("/login")} className="text-sm text-[var(--text-muted)] hover:text-white transition-colors">Login</button>
            <Button onClick={() => navigate("/login")} className="btn btn-primary text-sm px-4 py-2 rounded-lg">
      ...
      <main className="relative z-10">
        <section className="max-w-4xl mx-auto px-6 pt-24 pb-20 text-center">
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            <motion.div variants={fadeUp} className="inline-flex mb-6">
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium text-[var(--amber)] border border-[var(--amber)]/20 bg-[var(--amber)]/5">
                <span className="relative flex h-1.5 w-1.5">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[var(--amber)]/60" />
                  <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[var(--amber)]" />
                </span>
                V1.0 NOW LIVE
              </span>
            </motion.div>
            <motion.h1 variants={fadeUp} className="text-4xl md:text-6xl font-bold text-white mb-5 tracking-tight leading-[1.1]">
              ...
              <span className="text-[var(--amber)]">The System Is.</span>
            </motion.h1>
            <motion.p variants={fadeUp} className="text-base mb-8 text-[var(--text-muted)] max-w-lg mx-auto leading-relaxed">
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 justify-center items-center">
              <button onClick={() => navigate("/login")} className="btn btn-primary px-8 py-2.5 w-full sm:w-auto text-sm rounded-lg">Get Started Free</button>
              <button onClick={() => navigate("/dashboard")} className="btn btn-outline px-8 py-2.5 w-full sm:w-auto text-sm rounded-lg">Live Demo</button>
            </motion.div>
          </motion.div>
        </section>

        <section className="max-w-4xl mx-auto px-6 pb-16">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="border border-[var(--border)]/50 rounded-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[{ label: "Total PnL", value: "$17.26", icon: TrendingUp }, { label: "Total APY", value: "7,940", icon: Zap }, { label: "Number of Trades", value: "10,000", icon: BarChart3 }, { label: "Position Size", value: "1.5M", icon: Shield }].map((stat, i) => (
              <div key={i} className="text-center">
                <stat.icon className="w-4 h-4 text-[var(--text-muted)] mx-auto mb-2" />
        ...
        <section className="max-w-6xl mx-auto px-6 py-16">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div key={i} variants={fadeUp} onClick={() => navigate(feature.path)} className="group cursor-pointer p-6 border border-[var(--border)]/50 rounded-xl hover:border-[var(--amber)]/30 transition-all duration-300">
                <feature.icon className="w-5 h-5 text-[var(--amber)] mb-3" />
        ...
        <section className="max-w-6xl mx-auto px-6 py-16">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="mb-10">
              <h2 className="text-2xl font-bold text-white mb-2">Trusted by Traders</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={i} variants={fadeUp} className="p-6 border border-[var(--border)]/50 rounded-xl">
                  <div className="flex gap-0.5 mb-3">
                    {[1, 2, 3, 4, 5].map((s) => <Star key={s} className="w-3.5 h-3.5 text-[var(--amber)]" style={{ fill: "var(--amber)" }} />)}
                  </div>
                  <p className="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">"{t.quote}"</p>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[var(--surface-elevated)] flex items-center justify-center text-xs font-semibold text-white">{t.initial}</div>
                    <div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={i} variants={fadeUp} className={`relative p-6 border rounded-xl ${plan.popular ? "border-[var(--amber)]/50" : "border-[var(--border)]/50"}`}>
                  {plan.popular && <div className="absolute -top-2.5 left-6 px-3 py-0.5 bg-[var(--amber)] text-black text-[10px] font-bold rounded-full">Most Popular</div>}
                  ...
                  <button onClick={() => navigate("/login")} className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all ${plan.popular ? "bg-[var(--amber)] text-black hover:bg-[var(--amber)]/90" : "border border-[var(--border)] text-white hover:bg-white/5"}`}>{plan.cta}</button>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        <section className="max-w-4xl mx-auto px-6 py-16">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="border border-[var(--amber)]/30 rounded-xl p-10 text-center bg-[var(--amber)]/5">
            <h2 className="text-2xl font-bold text-white mb-2">Ready to Automate Your Success?</h2>
            ...
            <button onClick={() => navigate("/login")} className="inline-flex items-center gap-2 px-6 py-2.5 text-sm bg-[var(--amber)] text-black rounded-lg font-semibold hover:bg-[var(--amber)]/90 transition-all">
        ...
        <section className="max-w-2xl mx-auto px-6 py-16">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.h2 variants={fadeUp} className="text-xl font-bold text-white mb-6">Frequently Asked Questions</motion.h2>
            <div className="space-y-2">
              {[
                ...
                <motion.details key={i} variants={fadeUp} className="border border-[var(--border)]/50 rounded-lg group">
                  <summary className="flex items-center justify-between p-3 cursor-pointer text-sm font-medium text-white hover:text-[var(--amber)] transition-colors">
                    {faq.q}
