...
export function StrategyBuilderContent({ embedded = false, onClose, onSaved }: StrategyBuilderContentProps) {
  ...
  const blockTypes: { type: StrategyBlock["type"]; icon: any; color: string }[] = [
    { type: "market", icon: Database, color: "text-[#E8A20E]" },
    { type: "condition", icon: Activity, color: "text-[#28A745]" },
    { type: "indicator", icon: Layers, color: "text-[#E8A20E]" },
    { type: "risk", icon: ShieldCheck, color: "text-[#E8A20E]" },
    { type: "trade", icon: Zap, color: "text-[#E8A20E]" },
    { type: "exit", icon: ChevronRight, color: "text-[#DC3545]" },
  ];
              ...
              <div className="grid grid-cols-1 gap-2">
                {blockTypes.map(bt => (
                  <button
                    ...
                    className="flex items-center gap-3 p-3 rounded-lg bg-[#151B23] border border-[#252B35] hover:border-[#E8A20E] transition-all text-left group"
                  >
                    ...
                    <Plus className="w-3 h-3 ml-auto text-[#64748B] group-hover:text-[#E8A20E]" />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bloomberg-panel p-6">
              ...
              <div className="space-y-3">
                {strategiesQuery.data?.slice(0, 5).map(s => (
                  <div key={s.id} className="p-3 rounded-lg bg-black/20 border border-white/5 hover:border-[#E8A20E]/50 transition-all">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-md text-[#64748B] hover:text-[#E8A20E] hover:bg-[#E8A20E]/10 transition-colors disabled:opacity-50"
                      >
            ...
            <div className="bloomberg-panel min-h-[500px] flex flex-col">
              <div className="p-4 border-b border-[#252B35] flex items-center justify-between bg-black/20">
                ...
                <div className="flex bg-[#151B23] rounded-lg p-1">
                   <button onClick={() => setBuilderMode("blocks")} className={`px-3 py-1 text-[10px] font-bold rounded ${builderMode === "blocks" ? "bg-[#E8A20E] text-white" : "text-[#64748B]"}`}>BLOCKS</button>
                   <button onClick={() => setBuilderMode("visual")} className={`px-3 py-1 text-[10px] font-bold rounded ${builderMode === "visual" ? "bg-[#E8A20E] text-white" : "text-[#64748B]"}`}>IF/THEN</button>
                   <button onClick={() => setBuilderMode("ensemble")} className={`px-3 py-1 text-[10px] font-bold rounded ${builderMode === "ensemble" ? "bg-[#E8A20E] text-white" : "text-[#64748B]"}`}>ENSEMBLE</button>
                   <button onClick={() => setShowHistory((v) => !v)} className={`px-3 py-1 text-[10px] font-bold rounded ${showHistory ? "bg-[#E8A20E] text-white" : "text-[#64748B]"}`}><GitCompare className="w-3 h-3 inline mr-1" />HISTORY</button>
                   <button onClick={() => critiqueMutation.mutate({ rule: buildConfig().rule })} className={`px-3 py-1 text-[10px] font-bold rounded ${critiqueMutation.isPending ? "opacity-50" : "hover:bg-[#28A745]/20 text-[#28A745] border border-[#28A745]/30"}`}><ShieldCheck className="w-3 h-3 inline mr-1" />AI REVIEW</button>
                 </div>
               </div>
               ...
               {showHistory && (
                 <div className="border-b border-[#252B35] p-4 bg-black/20 space-y-3">
                   <div className="flex items-center justify-between">
                     ...
                     {versions.length >= 2 && compareIdx === null && (
                       <button onClick={() => setCompareIdx([versions.length - 2, versions.length - 1])} className="text-[10px] text-[#E8A20E] hover:underline">Compare last two</button>
                     )}
                   </div>
                ...
                {critiqueMutation.data && (
                  <div className="border-b border-[#252B35] p-4 bg-black/20">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-bold text-[#28A745] uppercase tracking-widest">Risk Reviewer Agent</span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {((critiqueMutation.data.findings as any[]) || []).map((f, i) => (
                        <div key={i} className="flex items-start gap-2 text-xs">
                          <span className={`px-1.5 py-0.5 rounded font-bold uppercase ${f.severity === "high" ? "bg-[#DC3545]/30 text-[#DC3545]" : f.severity === "medium" ? "bg-[#E8A20E]/30 text-[#F5B80B]" : "bg-[#64748B]/30 text-[#94A3B8]"}`}>{f.severity}</span>
                          <div><b className="text-white">{f.title}</b> <span className="text-[#94A3B8]">â€” {f.detail}</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {builderMode === "visual" ? (
                  ...
                  <div className="max-w-2xl mx-auto space-y-5">
                    <p className="text-xs text-[#64748B]">Combine 2â€“3 saved strategies into one bot. A trade fires only when the chosen number of sub-strategies agree (vote).</p>
                    <div>
                      ...
                      <div className="flex gap-2 mt-2">
                        {(["all", "majority", "any"] as const).map((v) => (
                          <button key={v} onClick={() => setEnsembleVote(v)} className={`px-3 py-1 text-[10px] font-bold rounded ${ensembleVote === v ? "bg-[#E8A20E] text-white" : "bg-[#151B23] text-[#64748B] border border-[#252B35]"}`}>{v === "all" ? "ALL agree" : v === "majority" ? "MAJORITY" : "ANY"}</button>
                        ))}
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      {((strategiesQuery.data || []) as any[]).filter((s) => s.config?.rule).map((s) => {
                        ...
                        return (
                          <label key={s.id} className="flex items-center gap-3 bg-[#151B23] border border-[#252B35] rounded-lg p-3 cursor-pointer hover:border-[#E8A20E]/50">
                        ...
                        {blocks.map((block, index) => {
                          ...
                          return (
                            <div key={block.id} className="relative group">
                              ...
                              <div className="flex gap-4 items-start bg-[#151B23] border border-[#252B35] p-4 rounded-xl group-hover:border-[#E8A20E]/50 transition-all">
                                ...
                                <div className="flex-1 space-y-1">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1">
                                      <button onClick={() => moveBlock(block.id, -1)} disabled={index === 0} className="text-[#64748B] hover:text-[#22BFC8] disabled:opacity-25 transition-colors" aria-label="Move up"><ArrowUp className="w-3 h-3" /></button>
                                      <button onClick={() => moveBlock(block.id, 1)} disabled={index === blocks.length - 1} className="text-[#64748B] hover:text-[#22BFC8] disabled:opacity-25 transition-colors" aria-label="Move down"><ArrowDown className="w-3 h-3" /></button>
                                      <button onClick={() => removeBlock(block.id)} className="text-[#64748B] hover:text-[#DC3545] transition-colors"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                  </div>
                                  <Input 
                                    value={block.value}
        ...
        <div className="fixed inset-0 z-[90] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHistory(false)}>
          <div className="w-full max-w-2xl bg-[#151B23] border border-[#252B35] rounded-xl max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#252B35]">
              ...
              <button onClick={() => setShowHistory(false)} className="text-[#64748B] hover:text-white">âœ•</button>
            </div>
                    ...
                    <div className="space-y-1">
                      {versions.map((v, i) => (
                        <div key={i} className="flex items-center gap-3 p-2 bg-black/20 rounded-lg">
                          ...
                          <button onClick={() => setDiffA(i)} className={`text-[10px] px-2 py-0.5 rounded ${diffA === i ? "bg-[#E8A20E] text-white" : "bg-white/5 text-[#94A3B8]"}`}>A</button>
                          <button onClick={() => setDiffB(i)} className={`text-[10px] px-2 py-0.5 rounded ${diffB === i ? "bg-[#E8A20E] text-white" : "bg-white/5 text-[#94A3B8]"}`}>B</button>
                        </div>
                      ))}
                    </div>
                    {diffLines.length > 0 && (
                      <div className="mt-3 p-3 bg-black/40 rounded-lg font-mono text-[11px] overflow-x-auto">
                        <p className="text-[#64748B] mb-2">Diff: v{(diffA ?? 0) + 1} â†’ v{(diffB ?? 0) + 1}</p>
                        {diffLines.map((l, i) => (
                          <div key={i} className={l.kind === "add" ? "text-[#28A745]" : l.kind === "del" ? "text-[#DC3545]" : "text-[#64748B]"}>
