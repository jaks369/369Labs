    ...
    <div className="min-h-screen bg-[#151B23] p-6">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          ...
          <Bell className="w-6 h-6 text-[#E8A20E]" />
        </div>
        ...
        {settingsQuery.isLoading ? (
          ...
          <div className="bg-[#151B23] border border-[#252B35] rounded-xl p-6 space-y-6">
            {toggles.map(t => (
              <div key={t.label} className="flex items-center justify-between py-4 border-b border-[#252B35] last:border-0">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${t.value ? "bg-[#E8A20E]" : "bg-[#252B35]"}`}
                >
            ...
            <div className="flex items-center gap-4 pt-2">
              <Button onClick={handleSave} disabled={saveMutation.isPending} className="bg-[#E8A20E] hover:bg-[#E8A20E] text-white">
                {saveMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                Save Settings
              </Button>
              {saved && (
                <span className="flex items-center gap-1 text-[#28A745] text-sm font-bold">
